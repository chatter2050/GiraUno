...
*Setup instructions for a gitian build of Girauno using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Girauno
...
- [Building Girauno](#building-girauno)
...
Clone the git repositories for girauno and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/girauno/girauno
```
...
In [doc/release-process.md](release-process.md) in the girauno repository under 'Fetch and build inputs'.
...
To build Girauno (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the girauno repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/girauno/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/girauno/girauno
...
For example:
```bash
URL=https://github.com/laanwj/girauno.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit girauno=${COMMIT} --url girauno=${URL} ../girauno/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit girauno=${COMMIT} --url girauno=${URL} ../girauno/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/girauno-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/girauno-build.assert
```
...
[girauno/gitian.sigs](https://github.com/girauno/gitian.sigs/) repository, or
